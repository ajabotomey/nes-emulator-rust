...
fn bg_palette(ppu: &NesPPU, tile_column: usize, tile_row: usize) -> [u8;4] {
    ...
    let attr_table_byte = ppu.vram[0x03c0 + attr_table_idx];

    let palette_idx = match (tile_column % 4 / 2, tile_row % 4 / 2) {
        (0, 0) => attr_table_byte & 0b11,
        (1, 0) => (attr_table_byte >> 2) & 0b11,
        (0, 1) => (attr_table_byte >> 4) & 0b11,
        (1, 1) => (attr_table_byte >> 6) & 0b11,
